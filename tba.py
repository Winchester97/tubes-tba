def tokenisasi(string):
    ...
    def state_initial(string):
        proposisi = ['p', 'q', 'r', 's']
        char = string[0]
        if char in proposisi:
            return state_pqrs(string[1:])
        elif char == 'n':
            return state_n(string[1:])
        elif char == 'a':
            return state_a(string[1:])
        elif char == 'o':
            return state_o(string[1:])
        elif char == 'x':
            return state_x(string[1:])
        elif char == 'i':
            return state_i(string[1:])
        elif char == 't':
            return state_t(string[1:])
        elif char == '(':
            return state_bukakurung()
        elif char == ')':
            return state_tutupkurung()
        else:
            return state_error()
    tokens = []
    word = ''
